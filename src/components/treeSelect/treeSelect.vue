<template>
  <el-select :value="valueTitle" :size="size" :disabled="disabled" :clearable="clearable" :placeholder="placeholder"
             @clear="clearHandle" ref="selectblur">
...
<script>
  export default {
    ...
    created() {
      if (this.url !== null) {
        // 判断是否是农场列表/farm/baseFarm/list，因为农场中farmId===1后面逻辑无法执行
        if (this.url.indexOf('baseFarm') > 0) {
          this.isFarm = true
        } else {
          this.isFarm = false
        }
    ...
    methods: {
      ...
      handleNodeClick(node) {
        ...
        if (this.isOnlySelectLeaf && node.children !== undefined && node.children.length > 0) {
