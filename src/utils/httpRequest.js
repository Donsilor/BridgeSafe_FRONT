...
const BASE_URL = process.env.NODE_ENV !== 'production' ? process.env.VUE_APP_BASE_API : process.env.VUE_APP_BASE_API
// 对面暴露的基础请求路径
...
axios.interceptors.request.use(config => {
  ...
  // 请求地址处理
  if (!config.noBASE_URL) {
    config.url = BASE_URL + config.url
  }
  const type = config.method
  const defaults = {}
  const arrayFormat = config.headers.arrayFormat || 'indices'
  if(type === 'post' && config.contentType === 'json'){
    config.headers['Content-Type'] = 'application/json; charset=utf-8'
    config.data = config.data
  }else if (type === 'post' && config.headers['Content-Type'] === 'application/x-www-form-urlencoded; charset=utf-8') {
    // post请求参数处理
