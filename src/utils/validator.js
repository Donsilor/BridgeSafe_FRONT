...
var isIntegerGtZero = (rule, value, callback) => {
  if (value && !validate.isIntegerGtZero(value)) {
    callback(new Error('整数必须大于0!'))
  } else {
    callback()
  }
 ...
 * @param callback
 */
var isFloatGtZero1 = (rule, value, callback) => {
  if (value && !validate.isFloatGtZero1(value)) {
    callback(new Error('浮点数必须大于零且只能包含一位小数!'))
  } else {
    callback()
  }
}
...
/**
 * 浮点数保留2位小数且大于等于0小于等于1
 * @param rule
 * @param value
 * @param callback
 */
var isFloatGtZeroLtOne2 = (rule, value, callback) => {
  if (value && !validate.isFloatGtZero2(value)) {
    callback(new Error('浮点数必须大于等于0小于等于1且只能包含2位小数!'))
