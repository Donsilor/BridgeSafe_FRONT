<template>
  <el-dialog :title="title" :close-on-click-modal="false" v-dialogDrag width="700px" :visible.sync="visible">
    <el-form :model="inputForm" status-icon :rules="dataRule" v-loading="loading" ref="inputForm" @keyup.enter.native="inputFormSubmit()" label-width="120px" @submit.native.prevent>
      <el-row :gutter="15">
        <el-col :span="24">
        ...
        <!-- <el-col :span="24">
          <el-form-item label="是否系统数据" prop="sysData" :rules="[{ required: true, message: '是否系统数据不能为空', trigger: 'blur' }]">
        ...
        <!-- <el-col :span="24">
          <el-form-item label="角色状态" prop="useable" :rules="[{ required: true, message: '角色状态不能为空', trigger: 'blur' }]">
            <el-select v-model="inputForm.useable" placeholder="请选择" style="width: 100%">
              <el-option v-for="item in this.$dictUtils.getDictList('enable_status')" :key="item.value" :label="item.label" :value="item.value"> </el-option>
            </el-select>
            <div class="help-block">“启用”代表此数据可用，“停用”则表示此数据不可用</div>
          </el-form-item>
        </el-col> -->
        <el-col :span="24">
  ...
  data() {
    return {
      ...
      dataRule: {
        ...
        enname: [{ required: true, message: '角色英文名称不能为空', trigger: 'blue' }],
      },
    };
  },
  methods: {
      ...
      this.$refs['inputForm'].validate((valid) => {
        if (valid) {
          ...
          this.$http({
            url: `/sys/role/save`,
            method: 'post',
            data: { ...this.inputForm, enname: this.inputForm.name },
          }).then(({ data }) => {
            this.loading = false;
            if (data && data.success) {
