        ...
        <el-col :span="12">
          <el-form-item label="机构类型" prop="type">
            <el-select v-model="inputForm.type" placeholder="请选择" style="width: 100%">
              <el-option v-for="item in this.$dictUtils.getDictList('sys_office_type')" :key="item.value" :label="item.label" :value="item.value"> </el-option>
            </el-select>
          </el-form-item>
        </el-col>
        <!-- <el-col :span="12">
          <el-form-item label="机构等级" prop="grade">
            <el-select v-model="inputForm.grade" placeholder="请选择" style="width: 100%">
              <el-option v-for="item in this.$dictUtils.getDictList('sys_office_grade')" :key="item.value" :label="item.label" :value="item.value"> </el-option>
            </el-select>
          </el-form-item>
        </el-col> -->
        <el-col :span="12">
          <el-form-item label="机构性质" prop="property">
            <el-select v-model="inputForm.property" placeholder="请选择" style="width: 100%">
              <el-option v-for="item in this.$dictUtils.getDictList('office_property')" :key="item.value" :label="item.label" :value="item.value"> </el-option>
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="机构位置" prop="area.id">
            <SelectTree
              ref="areaTree"
              :props="{
                value: 'id', // ID字段名
  ...
  data() {
    return {
      ...
      inputForm: {
        ...
        grade: '1', // 机构等级（1：一级；2：二级；3：三级；4：四级）
        address: '', // 联系地址
        zipCode: '', // 邮政编码
        master: '', // 负责人
        phone: '', // 电话
      ...
      dataRule: {
        ...
        grade: [{ required: true, message: '机构等级不能为空', trigger: 'blur' }],
        type: [{ required: true, message: '机构类型不能为空', trigger: 'blur' }],
        // code: [{ max: 50, message: '最大长度不能超过50个字符', trigger: 'blur' }],
        useable: [{ required: true, message: '是否可用不能为空', trigger: 'blur' }],
        property: [{ required: true, message: '机构性质不能为空', trigger: 'blur' }],
      },
    };
  },
  components: {
