    ...
    <el-tabs v-model="activeName" @tab-click="handleClick" class="page">
      <el-tab-pane v-for="item in rowList" :key="item.id" :label="item.diseaseTypeName" :name="item.id">
        <div class="diseaseList">
          ...
          <div v-for="(item, i) in data.inspectDailyInstanceDiseasePropertyList" :key="i">{{ item.propKey }} : {{ item.propValue }}</div>
        </div>
...
export default {
  ...
  methods: {
    ...
    refreshList() {
      ...
      this.loading = true;
      this.$http({
        url: `/inspectioninstancedailydisease/inspectDailyInstanceDisease/queryById?id=${this.activeName}`,
        method: 'get',
        params: {},
      })
        .then(({ data }) => {
          if (data && data.success) {
            this.data = data.inspectDailyInstanceDisease;
            this.loading = false;
          }
        })
        .catch(() => {
