<template>
  <div>
    <el-form :inline="true" v-show="isSearchCollapse" class="query-form" ref="searchForm" :model="searchForm" @keyup.enter.native="refreshList()" @submit.native.prevent>
      ...
      <el-form-item prop="evaluate.id">
        <el-select size="small" v-model="searchForm.evaluate.id" placeholder="任务评分" style="width: 100%">
          <el-option
            v-for="item in [
    ...
    <el-row>
      <el-tabs v-model="activeName" type="card" @tab-click="handleClick">
        ...
        <el-tab-pane label="待巡查" name="WAIT_FOR_INSPECT"></el-tab-pane>
        <el-tab-pane label="巡查中" name="INSPECTING"></el-tab-pane>
    ...
    <el-table table-layout="auto" :data="dataList" border @selection-change="selectionChangeHandle" v-loading="loading" class="tableAuto">
      ...
      <el-table-column prop="maintainOrg.name" label="养护单位" width="150"> </el-table-column>
      ...
      <el-table-column prop="planOrgNames" label="计划巡查组" :width="flexWidth('planOrgNames', dataList, '计划巡查组')"> </el-table-column>
      <el-table-column prop="planInspectorNames" label="计划巡查人员" :width="flexWidth('planInspectorNames', dataList, '计划巡查人员')"> </el-table-column>
      <el-table-column prop="planTimeStr" label="计划任务时间" :width="flexWidth('planTimeStr', dataList, '计划任务时间')"> </el-table-column>
      <el-table-column prop="factEndTime" label="巡查结束时间" :width="flexWidth('factEndTime', dataList, '巡查结束时间')"> </el-table-column>
      <el-table-column prop="factInspectorNames" label="实际巡查人员" :width="flexWidth('factInspectorNames', dataList, '实际巡查人员')"> </el-table-column>
      ...
      <el-table-column prop="evaluate.name" label="任务评分" :width="flexWidth('evaluate.name', dataList, '任务评分')"> </el-table-column>
      ...
      <el-table-column fixed="right" header-align="center" align="center" :width="activeLabel == '待分配' ? 210 : 100" label="操作">
        <template slot-scope="scope">
          ...
          <el-button v-if="activeLabel == '待巡查' || activeLabel == '巡查中'" type="text" size="small" @click="batchTerminate(scope.row.id)">召回</el-button>
...
export default {
  ...
  methods: {
    ...
    refreshList() {
      this.loading = true;
      this.$http({
        url: '/dailyinspectruleinstance/inspectRulesInstance/list',
        method: 'get',
        params: {
          pageNo: this.pageNo,
    ...
    del(id) {
      ...
      this.$confirm(`确定删除所选项吗?`, '提示', {
        ...
        this.loading = true;
        this.$http({
          url: '/dailyinspectruleinstance/inspectRulesInstance/batchDelete',
          method: 'delete',
          params: { ids: ids },
        })
          .then(({ data }) => {
    ...
    batchAllot(id) {
      let ids =
        ...
        this.dataListSelections
          ...
          .join(',');
      this.$http({
        url: '/dailyinspectruleinstance/inspectRulesInstance/preAllotCheck',
        params: { ids },
      }).then(({ data }) => {
        if (data && data.success) {
          this.$confirm(data.prompt || `确定分配所选项吗?`, '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning',
          }).then(() => {
            this.loading = true;
            this.$http({
              url: '/dailyinspectruleinstance/inspectRulesInstance/batchAllot',
              method: 'post',
              params: { ids },
            })
              .then(({ data }) => {
    ...
    batchTerminate(id) {
      ...
      this.$confirm(`任务召回后将自动终止，是否继续对所选项执行召回操作?`, '提示', {
        ...
        this.loading = true;
        this.$http({
          url: '/dailyinspectruleinstance/inspectRulesInstance/batchTerminate',
          method: 'post',
          params: { ids: ids },
        })
          .then(({ data }) => {
    ...
    audit(row) {
      ...
      this.$http({
        ...
        if (data && data.success) {
          const value = data.list.filter((v) => v.code === 'daily_auto_audit')[0].value1;
          if (row.diseaseCount) {
            if (value === '1') {
